<template>
	<view class="w-table-box">
		<view
			v-if="loading"
			class="w-table-loading-box"
			:style="{
		...
		>
			<view class="w-table-header" :style="{ borderBottom: '1px solid' + borderColor }">
				<view
					class="w-table-header-item"
					v-if="enableCheck"
					:style="{
				...
				>
					...
					<view class="w-table-header-icon" v-if="autoSortShow(index)">
						<text
...
.w-table-scroll-view::-webkit-scrollbar {
	display: none;
	/* #ifdef MP-WEIXIN */
	width: 0;
	height: 0;
